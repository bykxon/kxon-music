...
(function () {
  ...
  if (!db) {
    ...
    var _t = setInterval(function () {
      db = window.db; _r++;
      if (db) { clearInterval(_t); boot(); }
      else if (_r > 30) { clearInterval(_t); console.error('[PL] No db'); }
    }, 300);
    return;
  }
  boot();
  ...
  function boot() {
    ...
    window._loadPlaylists = async function () {
      ...
      try {
        ...
        all = r.data || [];
        console.log('[PL] Loaded:', all.length);
        applyFilters();
        updateKPIs();
      } catch (e) {
        console.error('[PL] Load error:', e);
        grid.innerHTML = errHtml('Error al cargar playlists');
      }
      loading = false;
    };
    ...
    function updateKPIs() {
      ...
      var e2 = $('plStatTracks'); if (e2) e2.textContent = t;
      var e3 = $('plStatPublic'); if (e3) e3.textContent = p;
    }
    ...
    function renderGrid() {
      ...
      for (var i = 0; i < filtered.length; i++) {
        ...
        if (img) h += '<img class="kx-pl-card-img" src="' + esc(img) + '" alt="" loading="lazy">';
        else h += '<div class="kx-pl-card-fallback"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M9 18V5l12-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="18" cy="16" r="3"/></svg><span class="kx-pl-card-fallback-text">PLAYLIST</span></div>';
        if (p.publica) h += '<span class="kx-pl-card-badge kx-pl-card-badge--public"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><circle cx="12" cy="12" r="10"/><path d="M2 12h20"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/></svg>Pública</span>';
        h += '<div class="kx-pl-card-overlay"><button class="kx-pl-card-play" data-action="play-playlist" data-id="' + p.id + '"><svg viewBox="0 0 24 24" fill="currentColor"><polygon points="5 3 19 12 5 21 5 3"/></svg></button><span class="kx-pl-card-count">' + n + ' cancion' + (n !== 1 ? 'es' : '') + '</span></div>';
        ...
        h += '</div>';
        h += '<div class="kx-pl-card-body"><div class="kx-pl-card-title">' + nm + '</div><div class="kx-pl-card-sub"><span>' + n + ' cancion' + (n !== 1 ? 'es' : '') + '</span><span class="kx-pl-card-sub-dot"></span><span>' + ago(dt) + '</span></div></div>';
        h += '</div>';
      }
      g.innerHTML = h;
    }
    ...
    // ── TEMPLATES ──
    function skelGrid(el, c) { var h = ''; for (var i = 0; i < c; i++) h += '<div class="kx-pl-skeleton"><div class="kx-pl-skeleton-img"></div><div class="kx-pl-skeleton-body"><div class="kx-pl-skeleton-line"></div><div class="kx-pl-skeleton-line kx-pl-skeleton-line--short"></div></div></div>'; el.innerHTML = h; }
    function skelTracks(el, c) { var h = ''; for (var i = 0; i < c; i++) h += '<div class="kx-pl-skeleton-track"><div class="kx-pl-skeleton-track-num"></div><div class="kx-pl-skeleton-track-cover"></div><div class="kx-pl-skeleton-track-bar"></div><div class="kx-pl-skeleton-track-dur"></div></div>'; el.innerHTML = h; }
    ...
    function emptyFilter() { return '<div class="kx-pl-empty"><div class="kx-pl-empty-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/></svg></div><div class="kx-pl-empty-title">Sin resultados</div></div>'; }
    function errHtml(m) { return '<div class="kx-pl-empty"><div class="kx-pl-empty-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><circle cx="12" cy="12" r="10"/><path d="M12 8v4"/><path d="M12 16h.01"/></svg></div><div class="kx-pl-empty-title">' + esc(m) + '</div></div>'; }
    ...
    window._openPlaylistDetail = async function (id) {
      ...
      try {
        ...
        var bg = $('plDetailHeroBg'); if (bg) bg.style.backgroundImage = img ? 'url(' + img + ')' : 'none';
        ...
        var up = $('plDetailUpdated'); if (up) up.textContent = ago(r.data.updated_at || r.data.created_at);
        ...
        console.error('[PL] Detail error:', e);
        toast('Error al cargar playlist', 'error');
      }
    };

    /* ══════════════════════════════════
       ★★★ LOAD SONGS — DIAGNOSTIC VERSION
       ══════════════════════════════════ */
    async function loadSongs(playlistId) {
      var tl = $('plTrackList');
      console.log('[PL] ═══ Loading songs for playlist:', playlistId);

      try {
        // STEP 1: Get raw playlist_canciones
        console.log('[PL] Step 1: Fetching playlist_canciones...');
        var r1 = await db.from('playlist_canciones')
          .select('*')
          .eq('playlist_id', playlistId);

        if (r1.error) {
          console.error('[PL] Step 1 ERROR:', r1.error);
          throw r1.error;
        }

        console.log('[PL] Step 1 result:', JSON.stringify(r1.data, null, 2));

        var pcRows = r1.data || [];
        if (!pcRows.length) {
          console.log('[PL] No songs in playlist');
          plSongs = [];
          renderSongs(tl);
          updateMeta();
          return;
        ...
        // ★ DETECT column name for song ID
        var firstRow = pcRows[0];
        console.log('[PL] First row columns:', Object.keys(firstRow));

        // Try different possible column names
        var songIdCol = null;
        if (firstRow.cancion_id !== undefined) songIdCol = 'cancion_id';
        else if (firstRow.song_id !== undefined) songIdCol = 'song_id';
        else if (firstRow.track_id !== undefined) songIdCol = 'track_id';
        else if (firstRow.canciones_id !== undefined) songIdCol = 'canciones_id';

        console.log('[PL] Song ID column detected:', songIdCol);

        if (!songIdCol) {
          console.error('[PL] ★ Cannot find song ID column! Available columns:', Object.keys(firstRow));
          if (tl) tl.innerHTML = errHtml('Error: columna de canción no encontrada. Columnas: ' + Object.keys(firstRow).join(', '));
          return;
        }

        // Get song IDs
        var songIds = [];
        var pcMap = {}; // map song_id -> pc row
        for (var i = 0; i < pcRows.length; i++) {
          var sid = pcRows[i][songIdCol];
          if (sid) {
            songIds.push(sid);
            pcMap[sid] = pcRows[i];
          }
        ...
        console.log('[PL] Song IDs:', songIds);
        ...
        // STEP 2: Fetch songs
        console.log('[PL] Step 2: Fetching canciones...');
        var r2 = await db.from('canciones')
          .select('*')
          .in('id', songIds);

        if (r2.error) {
          console.error('[PL] Step 2 ERROR:', r2.error);
          throw r2.error;
        }

        console.log('[PL] Step 2 result:', r2.data ? r2.data.length : 0, 'songs');
        if (r2.data && r2.data.length > 0) {
          console.log('[PL] First song columns:', Object.keys(r2.data[0]));
          console.log('[PL] First song:', JSON.stringify(r2.data[0], null, 2));
        }
        ...
        (r2.data || []).forEach(function (s) {
          songsMap[s.id] = s;
          // Try different album column names
          var albId = s.album_id || s.albumes_id || s.albumId;
          if (albId && albumIds.indexOf(albId) === -1) albumIds.push(albId);
        });

        // STEP 3: Fetch albums
        var albumsMap = {};
        if (albumIds.length) {
          console.log('[PL] Step 3: Fetching albums:', albumIds);
          var r3 = await db.from('albumes')
            .select('id, titulo, imagen_url')
            .in('id', albumIds);
          if (!r3.error && r3.data) {
            r3.data.forEach(function (a) { albumsMap[a.id] = a; });
            console.log('[PL] Albums loaded:', Object.keys(albumsMap).length);
          }
        }

        // STEP 4: Merge
        plSongs = [];
        // Maintain order from playlist_canciones
        for (var j = 0; j < pcRows.length; j++) {
          var pc = pcRows[j];
          var songId = pc[songIdCol];
          var song = songsMap[songId];
          if (!song) {
            console.warn('[PL] Song not found in canciones table:', songId);
            continue;
          }

          var albId2 = song.album_id || song.albumes_id || song.albumId;
          var album = albId2 ? albumsMap[albId2] : null;

          // Detect audio URL column
          var audioUrl = song.audio_url || song.archivo_url || song.url || song.file_url || '';

          plSongs.push({
            pcId: pc.id,
            posicion: pc.posicion || (j + 1),
            id: song.id,
            titulo: song.titulo || song.nombre || song.title || 'Sin título',
            audio_url: audioUrl,
            archivo_url: song.archivo_url || audioUrl,
        ...
        console.log('[PL] ★ Final merged songs:', plSongs.length);
        renderSongs(tl);
        updateMeta();

      } catch (e) {
        console.error('[PL] ★ Song load FATAL error:', e);
        console.error('[PL] Error name:', e.name);
        console.error('[PL] Error message:', e.message);
        console.error('[PL] Error details:', e.details);
        console.error('[PL] Error hint:', e.hint);
        if (tl) tl.innerHTML = errHtml('Error: ' + (e.message || 'desconocido'));
      }
    }

    function updateMeta() {
      var m = $('plDetailMeta'); if (m) m.textContent = plSongs.length + ' CANCION' + (plSongs.length !== 1 ? 'ES' : '');
      var s = $('plTracksSub'); if (s) s.textContent = plSongs.length + ' cancion' + (plSongs.length !== 1 ? 'es' : '') + ' en esta playlist';
      var sec = 0;
      for (var i = 0; i < plSongs.length; i++) {
        var d = plSongs[i].duracion;
        if (d && d !== '--:--') {
          var p = d.split(':');
          if (p.length === 2) sec += parseInt(p[0]) * 60 + parseInt(p[1]);
          else if (p.length === 3) sec += parseInt(p[0]) * 3600 + parseInt(p[1]) * 60 + parseInt(p[2]);
        }
      }
      var de = $('plDetailDuration'); if (de) de.textContent = Math.floor(sec / 60) + ' min';
    }

    function renderSongs(c) {
      if (!c) return;
      if (!plSongs.length) {
        c.innerHTML = '<li class="kx-pl-empty-tracks"><div class="kx-pl-empty-tracks-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M9 18V5l12-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="18" cy="16" r="3"/></svg></div><div class="kx-pl-empty-tracks-title">Playlist vacía</div><div class="kx-pl-empty-tracks-text">Agrega canciones desde Canciones o Álbumes</div></li>';
        return;
      }
      var h = '';
      for (var i = 0; i < plSongs.length; i++) {
        var s = plSongs[i], img = s.albumImg || PH;
    ...
    window._playPlaylistTrack = function (index) {
      ...
      var tracks = [];
      for (var i = 0; i < plSongs.length; i++) {
        var s = plSongs[i], url = s.audio_url || s.archivo_url;
        if (!url) continue;
        tracks.push({
          id: s.id, titulo: s.titulo,
          archivo_url: url, audio_url: url,
          imagen_url: s.albumImg || PH, cover: s.albumImg || PH,
      ...
      if (K.playTrackList) K.playTrackList(tracks, idx);
      else if (K.playTrack) {
    ...
    window._shufflePlaylist = function () {
      if (plSongs.length) window._playPlaylistTrack(Math.floor(Math.random() * plSongs.length));
    };
    ...
    window._openCreatePlaylist = function () {
      var m = $('modalPlaylist'); if (!m) { toast('Modal no encontrado', 'error'); return; }
    ...
    window._submitPlaylist = async function (evt) {
      ...
      var u = user(); if (!u) { toast('Inicia sesión', 'error'); return; }
      var titulo = ($('plNombre') || {}).value; if (titulo) titulo = titulo.trim();
      ...
      try {
        ...
        if (editId) {
          var r = await db.from('playlists').update({ titulo: titulo, descripcion: desc, publica: publica, updated_at: now }).eq('id', editId).eq('usuario_id', u.id);
          if (r.error) throw r.error;
          toast('Actualizada ✓');
          if (plData && plData.id === editId) {
            plData.titulo = titulo; plData.descripcion = desc; plData.publica = publica;
            var dt = $('plDetailTitle'); if (dt) dt.textContent = titulo;
            var dd = $('plDetailDesc'); if (dd) dd.textContent = desc || 'Sin descripción';
          }
        } else {
          var r2 = await db.from('playlists').insert({ usuario_id: u.id, titulo: titulo, descripcion: desc, publica: publica, created_at: now, updated_at: now });
          if (r2.error) throw r2.error;
          toast('¡Playlist creada! 🎶');
        }
        window._closePlaylistModal();
        await window._loadPlaylists();
      } catch (e) { console.error('[PL]', e); toast('Error: ' + (e.message || ''), 'error'); }
      if (btn) btn.disabled = false;
    };

    window._closePlaylistModal = function () { var m = $('modalPlaylist'); if (m) m.classList.remove('show'); };

    /* ══════════════════════════════════
       DELETE / REMOVE
       ══════════════════════════════════ */
    function showConfirm(t, tx, at, cb) {
      var o = $('plConfirmOverlay'), te = $('plConfirmTitle'), txe = $('plConfirmText'), ae = $('plConfirmAccept');
      if (te) te.textContent = t; if (txe) txe.textContent = tx; if (ae) ae.textContent = at;
      confirmCb = cb; if (o) o.classList.add('show');
    }
    function hideConfirm() { var o = $('plConfirmOverlay'); if (o) o.classList.remove('show'); confirmCb = null; }

    window._deletePlaylist = function (id) {
      var name = 'esta playlist';
      for (var i = 0; i < all.length; i++) if (all[i].id === id) { name = all[i].titulo; break; }
      showConfirm('¿Eliminar?', '"' + name + '" se eliminará.', 'Eliminar', async function () {
        try {
          await db.from('playlist_canciones').delete().eq('playlist_id', id);
          var r = await db.from('playlists').delete().eq('id', id);
          if (r.error) throw r.error;
          toast('Eliminada');
          if (plId === id) { var lv = $('plListView'), dv = $('plDetailView'); if (lv) lv.style.display = 'block'; if (dv) dv.classList.remove('show'); plId = null; plData = null; }
          await window._loadPlaylists();
    ...
    window._removeFromPlaylist = function (pcId) {
      showConfirm('¿Quitar?', 'Se quitará de esta playlist.', 'Quitar', async function () {
        try {
    ...
    window._backToPlaylists = function () {
      var lv = $('plListView'), dv = $('plDetailView');
      if (lv) lv.style.display = 'block'; if (dv) dv.classList.remove('show');
    ...
    window._openAddToPlaylist = function (cancionId, cancionTitulo) {
      console.log('[PL] _openAddToPlaylist called:', cancionId);
      var u = user(); if (!u) { toast('Inicia sesión', 'error'); return; }
      if (!cancionId) { toast('Canción no identificada', 'error'); return; }
    ...
    async function loadAddModal() {
      var list = $('addToPlList'); if (!list) return;
      list.innerHTML = '<div style="text-align:center;padding:24px;color:rgba(255,255,255,.3)">Cargando...</div>';
      ...
      try {
        ...
        for (var i = 0; i < pls.length; i++) {
          ...
          h += '<div class="kx-atp-item' + (already ? ' kx-atp-added' : '') + '"' + (already ? '' : ' data-action="add-to-pl" data-plid="' + pl.id + '"') + ' tabindex="0">';
        ...
        console.error('[PL]', e);
        list.innerHTML = '<div style="text-align:center;padding:24px;color:rgba(255,255,255,.3)">Error</div>';
      }
    }

    window._addSongToPlaylist = async function (playlistId) {
      var data = window._addToPlaylistData; if (!data) return;
      console.log('[PL] Adding song', data.cancionId, 'to playlist', playlistId);
      ...
      try {
        ...
        var insertData = { playlist_id: playlistId, cancion_id: data.cancionId, posicion: pos };
        console.log('[PL] Inserting:', insertData);

        var r = await db.from('playlist_canciones').insert(insertData);
        if (r.error) {
          if (r.error.message && (r.error.message.indexOf('unique') >= 0 || r.error.message.indexOf('duplicate') >= 0)) { toast('Ya está', 'error'); return; }
          throw r.error;
        }
    ...
    if (panel) {
      panel.addEventListener('click', function (e) {
        ...
        if (card && !t.closest('[data-action]')) { window._openPlaylistDetail(card.getAttribute('data-id')); return; }

        var a = t.closest('[data-action]');
        if (a) {
          var act = a.getAttribute('data-action');
          e.stopPropagation();
          if (act === 'play-playlist') { var pid = a.getAttribute('data-id'); (async function () { await window._openPlaylistDetail(pid); setTimeout(window._playAllPlaylist, 400); })(); return; }
          if (act === 'delete-playlist') { window._deletePlaylist(a.getAttribute('data-id')); return; }
          if (act === 'remove-track') { window._removeFromPlaylist(a.getAttribute('data-pcid')); return; }
          if (act === 'create-playlist') { window._openCreatePlaylist(); return; }
        }
        ...
        if (fb) {
          panel.querySelectorAll('.kx-pl-filter').forEach(function (f) { f.classList.remove('active'); f.setAttribute('aria-selected', 'false'); });
          fb.classList.add('active'); fb.setAttribute('aria-selected', 'true');
          filter = fb.getAttribute('data-filter');
          applyFilters(); return;
        }

        if (t.closest('#plSortBtn')) { var dd = $('plSortDropdown'); if (dd) dd.classList.toggle('show'); return; }

        var so = t.closest('.kx-pl-sort-option');
        if (so) {
          panel.querySelectorAll('.kx-pl-sort-option').forEach(function (s) { s.classList.remove('active'); s.setAttribute('aria-selected', 'false'); });
        ...
        if (t.closest('#plSearchClear')) { var inp = $('plSearchInput'); if (inp) { inp.value = ''; inp.focus(); } search = ''; t.closest('#plSearchClear').style.display = 'none'; applyFilters(); return; }
        if (t.closest('#plConfirmCancel')) { hideConfirm(); return; }
        if (t.closest('#plConfirmAccept') && confirmCb) { confirmCb(); return; }
        if (!t.closest('.kx-pl-sort-wrap')) { var dd3 = $('plSortDropdown'); if (dd3) dd3.classList.remove('show'); }
      });
      ...
      panel.addEventListener('keydown', function (e) {
        if (e.key === 'Enter' || e.key === ' ') {
          ...
          if (track) { e.preventDefault(); var idx = parseInt(track.getAttribute('data-index')); if (!isNaN(idx)) window._playPlaylistTrack(idx); }
        }
      });

      var si = $('plSearchInput');
      if (si) { var st; si.addEventListener('input', function () { var v = this.value.trim(); var cb = $('plSearchClear'); if (cb) cb.style.display = v ? 'flex' : 'none'; clearTimeout(st); st = setTimeout(function () { search = v; applyFilters(); }, 200); }); }
    ...
    document.addEventListener('click', function (e) {
      ...
      var ai = t.closest('[data-action="add-to-pl"]');
      if (ai) { window._addSongToPlaylist(ai.getAttribute('data-plid')); return; }
      var cl = t.closest('[data-action="create-from-modal"]');
      if (cl) { e.preventDefault(); window._closeAddToPlaylist(); window._openCreatePlaylist(); return; }
      if (!t.closest('.kx-pl-sort-wrap')) { var dd = $('plSortDropdown'); if (dd) dd.classList.remove('show'); }
    });
