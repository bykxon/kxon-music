/* ============================================
   ...
   + Soporte para notificaciones de EN VIVO
   ============================================ */
(function () {
    ...
    waitForKXON(function () { initNotifications(); });
    ...
    function initNotifications() {
        ...
        /* ══════════════════════════════════════════
           🎨 RENDERIZAR LISTA (con soporte live)
           ══════════════════════════════════════════ */
        function renderNotifications() {
            ...
            for (var i = 0; i < allNotifications.length; i++) {
                ...
                else if (n.tipo === 'live') { iconClass = 'icon-live'; iconEmoji = '🔴'; }
                else if (n.tipo === 'sub_confirmada') { iconEmoji = '🎫'; }
                else if (n.tipo === 'sub_rechazada') { iconEmoji = '❌'; }

                var timeAgo = getTimeAgo(n.created_at);

                h += '<div class="notif-item' + (n.leida ? '' : ' unread') + (n.tipo === 'live' ? ' notif-live' : '') + '" onclick="window._notifClick(\'' + n.id + '\',' + i + ')">';
        ...
        /* ══════════════════════════════════════════
           🖱️ CLICK EN NOTIFICACIÓN (con soporte live)
           ══════════════════════════════════════════ */
        window._notifClick = async function (notifId, idx) {
            ...
            if (n.tipo === 'live') {
                K.showPanel('envivo');
            } else if (n.link_panel) {
                K.showPanel(n.link_panel);
            } else if (n.tipo === 'compra_confirmada') {
                K.showPanel('archivo');
                ...
                K.showPanel('albumes');
                setTimeout(function () {
                    if (typeof window._openAlbum === 'function') window._openAlbum(n.data.album_id);
                }, 200);
            } else if (n.tipo === 'nuevo_video' && n.data && n.data.video_id) {
                K.showPanel('videos');
                setTimeout(function () {
                    if (typeof window._openVideoPlayer === 'function') window._openVideoPlayer(n.data.video_id);
                }, 200);
            } else if (n.tipo === 'sub_confirmada' || n.tipo === 'sub_rechazada') {
                K.showPanel('planes');
            }
        };
        ...
        K.sendNotification = async function (userId, tipo, titulo, mensaje, data) {
            try {
                ...
                await db.from('notificaciones').insert(insertData);
            } catch (e) {
        ...
        console.log('✅ dashboard-notifications.js cargado correctamente');
    }
