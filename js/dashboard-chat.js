/* ═══════════════════════════════════════════════════
   💬 KXON CHAT — PREMIUM REDESIGN 2026
   Namespace: kx-chat-*
...
(function () {
  ...
  function initChat() {
    ...
    function formatChatText(text) {
      text = esc(text);
      // Only allow http/https links — prevents javascript: protocol
      text = text.replace(/(https?:\/\/[^\s<]+)/g,
        '<a href="$1" target="_blank" rel="noopener noreferrer" class="kx-chat-link">$1</a>');
      return text;
    }
    ...
    K.loadChat = async function () {
      ...
      try {
        ...
        var list = document.getElementById('chatMessagesList');
        if (list) {
          list.innerHTML = buildErrorState(
      ...
      chatChannel = db.channel('chat-room')
        .on('postgres_changes', {
          ...
          for (var i = 0; i < chatMessages.length; i++) {
            if (chatMessages[i].id === msg.id) return;
          }
        ...
        .on('postgres_changes', {
          ...
          if (el) {
            el.style.animation = 'kxcMsgOut 0.25s ease forwards';
    ...
    function buildErrorState(title, text) {
      return '<div class="kx-chat-empty">' +
        '<div class="kx-chat-empty-icon">' +
          '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">' +
    ...
    function renderChat() {
      ...
      if (!chatMessages.length) {
        list.innerHTML = buildEmptyState(
          ...
          'Inicia la conversación con la comunidad KXON'
        );
        return;
      }
      ...
      for (var i = 0; i < chatMessages.length; i++) {
        ...
        if (msgDate !== lastDate) {
          h += '<div class="kx-chat-date-sep" aria-hidden="true">' +
    ...
    function buildMessageHTML(msg) {
      ...
      if (avatarUrl) {
        ...
        avatarContent =
          '<span class="kx-chat-avatar-letter">' + esc(initial) + '</span>';
      }

      var h = '<div class="kx-chat-msg' + (isMe ? ' kx-chat-msg--me' : '') + '"' +
              ' id="kx-chat-msg-' + esc(msg.id) + '"' +
              ' data-ts="' + esc(msg.created_at) + '">';

      h += '<div class="kx-chat-avatar">' + avatarContent + '</div>';
      h += '<div class="kx-chat-msg-content">';
      h += '<div class="kx-chat-msg-header">';
      h += '<span class="kx-chat-msg-name' + (isMe ? ' kx-chat-msg-name--me' : '') + '">' +
           esc(msg.usuario_nombre) + '</span>';
      h += '<span class="kx-chat-msg-time">' + time + '</span>';

      // Admin delete button
      if (K.isAdmin && !isMe) {
        h += '<button class="kx-chat-msg-delete" data-delete-msg="' + esc(msg.id) + '"' +
             ' title="Eliminar" aria-label="Eliminar mensaje">' +
    ...
    async function deleteMessage(msgId) {
      if (!K.isAdmin) return;
      try {
        var r = await db.from('chat_messages').delete().eq('id', msgId);
        if (r.error) throw r.error;
      } catch (e) {
    ...
    console.log('✅ dashboard-chat.js (v3 — Premium Redesign) cargado');
  }
